...
3. 如果注册成功，子域名将列在 `域名` 部分，并且 `当前 IP` 将是您当前用来访问 `duckdns.org` 的设备的公共 IP 地址。DuckDNS 添加程序将更新 IP 地址。
4. 在 DuckDNS 添加程序配置中执行以下操作：
    - 从 `duckdns.org` 处显示账户详情的页面顶部复制 DuckDNS 令牌并粘贴到 `token` 选项中。
...
此外，您需要配置 Home Assistant 核心来获取 SSL 证书。这是通过在您的 `configuration.yaml` 中设置以下配置来完成的，用于 [HTTP][HTTP] 集成配置：
...
以下选项是用于选项组：`lets_encrypt`。这些设置仅适用于 Let's Encrypt SSL 证书。
...
Let's Encrypt 生成的证书文件的名称。该文件用于 Home Assistant 添加程序的 SSL，建议保持文件名不变（`fullchain.pem`）以确保兼容性。

**注意**：_文件存储在 `/ssl/`，这是 Home Assistant 的默认路径_

#### 选项 `lets_encrypt.keyfile`

Let's Encrypt 生成的私钥文件的名称。私钥文件用于 Home Assistant 添加程序的 SSL，建议保持文件名不变（`privkey.pem`）以确保兼容性。
...
如果您在此处指定一个 URL，它将获取该资源指向的内容并将其用作地址。这可以通过使用类似于 https://api.ipify.org/ 或 https://ipv4.text.wtfismyip.com 的服务来获取地址。
...
如果您在此处指定一个 URL，它将获取该资源指向的内容并将其用作地址。这可以通过使用类似于 https://api6.ipify.org/ 或 https://ipv6.text.wtfismyip.com 的服务来获取地址。

### 选项：`token`

在 DuckDNS 账户主页顶部找到的 DuckDNS 身份验证令牌。令牌是修改您账户注册的子域名所必需的。
...
建议将您的 CNAME 的 TTL 值设置为较低的值，通常在 60 以下。
...
不要将您的自定义域名添加到 `domains` 数组中。对于证书创建，所有唯一的域名和别名都将被使用。

同样，请确保 dns-01 挑战可以到达 Duckdns。可能需要添加特定的 CNAME：
...
- 要登录，DuckDNS 需要来自以下服务的免费账户：Google、Github、Twitter 或 Persona。
- 免费的 DuckDNS 账户最多只能有五个子域名。
- 在撰写本文时，Duck DNS 自身的 IPv6 自动检测 [实际上不起作用][duckdns-faq]，但您可以使用 `ipv6` 的 URL 选项来解决这个问题，请继续阅读。
