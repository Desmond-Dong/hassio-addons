...
这个插件是一个简单的包装器，围绕着优秀的 [rtl_433](https://github.com/merbanan/rtl_433) 项目，通过 [支持的SDR棒之一](https://triq.org/rtl_433/HARDWARE.html) 接收无线传感器数据，解码并以多种格式输出，包括JSON和MQTT。无线传感器rtl_433主要理解在433.92 MHz、868 MHz、315 MHz、345 MHz和915 MHz ISM频段上传输的数据。
...
默认情况下，rtl_433将其接收到的数据打印到终端——你需要配置它以发布数据到MQTT，以便Home Assistant可以访问它，这可以通过配置文件中的一行来完成。

一旦你将rtl_433传感器数据放入MQTT，你将需要帮助Home Assistant发现并理解这些数据。你可以通过以下几种方式完成：

  * 手动配置HA中的`sensors`和`binary_sensors`，并将它们链接到从rtl_433出来的相应MQTT主题[https://www.home-assistant.io/integrations/sensor.mqtt/](https://www.home-assistant.io/integrations/sensor.mqtt/)，
...
要使用这个插件，你需要以下条件：
 ...
 2. 在一个插入SDR棒的主机上运行的Home Assistant OS。
...
## 安装
 ...
 1. 创建一个满足你需求的rtl_433配置文件。如果你在运行Home Assistant OS的计算机之外做这个，可能会更好，这样你可以自由地实验，迭代直到你得到一个工作良好的配置。见下文了解更多细节。

 2. 使用你认为合适的方法将配置文件上传到Home Assistant的"/config"目录（通过Samba插件、ssh/scp、文件编辑插件等）。
...
对于“零配置”设置，安装[Mosquitto代理](https://github.com/home-assistant/addons/blob/master/mosquitto/DOCS.md)插件。虽然其他代理可能也能工作，但它们没有经过测试，需要手动设置。插件安装后，启动或重启rtl_433和rtl_433_mqtt_autodiscovery插件以开始捕获已知的433 MHz协议。
...
在手动配置时，假设你打算将rtl_433数据放入Home Assistant，配置文件中需要指定的最绝对的最小信息是[MQTT连接和认证信息](https://triq.org/rtl_433/OPERATION.html#mqtt-output)：

```
output      mqtt://HOST:PORT,user=XXXX,pass=YYYYYYY
```

rtl_433默认监听433.92MHz，但如果那是你的需求，最好明确指定频率以避免混淆：
...
最后但同样重要的是，如果你决定使用MQTT自动发现脚本或插件，其文档建议将所有从rtl_433出来的数据转换为SI：

```
convert     si
```

假设你只有一个USB棒连接，并且rtl_433能够自动找到它，我们得到一个看起来像这样最小的rtl_433配置文件：
