...
这个插件是一个简单的包装器，围绕优秀的 [rtl_433](https://github.com/merbanan/rtl_433) 项目，通过 [支持的 SDR 捆绑设备之一](https://triq.org/rtl_433/HARDWARE.html) 接收无线传感器数据，解码并以多种格式输出，包括 JSON 和 MQTT。无线传感器 rtl_433 主要在 433.92 MHz、868 MHz、315 MHz、345 MHz 和 915 MHz ISM 频段上传输数据。
...
默认情况下，rtl_433 将接收到的数据打印到终端——你需要配置它以将数据发布到 MQTT，以便 Home Assistant 可以访问它，这可以通过配置文件中的一行来完成。
...
要使用这个插件，你需要以下条件：
 ...
 1. [支持 rtl_433 的 SDR 捆绑设备](https://triq.org/rtl_433/HARDWARE.html)。

 2. 在插入 SDR 捆绑设备的机器上运行 Home Assistant OS。

 3. 一些支持 rtl_433 的无线传感器。支持协议和设备的完整列表可以在 [rtl_433 的 README](https://github.com/merbanan/rtl_433/blob/master/README.md) 的“支持设备协议”部分找到。

## 安装

 1. 创建一个符合你需求的 rtl_433 配置文件。如果你在运行 Home Assistant OS 的计算机以外的计算机上这样做，可能会更好，这样你可以自由地实验并迭代，直到你找到一个工作良好的配置。请参阅下方更多详细信息。

 2. 使用你认为合适的方法将配置文件上传到 Home Assistant 的 "/config" 目录（通过 Samba 插件、ssh/scp、文件编辑器插件等）。

 3. 安装插件。

 5. 将你的 SDR 捆绑设备连接到运行插件的计算机。
...
`retain` 选项控制 MQTT 的 `retain` 标志是默认启用还是禁用。它可以通过在 `output` 设置中将 `retain` 设置为 `true` 或 `false` 来在单个无线电的基础上覆盖。
...
假设你只连接了一个 USB 捆绑设备并且 rtl_433 能够自动找到它，我们得到了一个看起来像这样最小化的 rtl_433 配置文件：
