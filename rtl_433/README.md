...
这个插件是一个简单的封装，围绕优秀的 [rtl_433](https://github.com/merbanan/rtl_433) 项目，通过 [支持的一种SDR转接器](https://triq.org/rtl_433/HARDWARE.html) 接收无线传感器数据，解码并以多种格式（包括JSON和MQTT）输出。无线传感器rtl_433主要在433.92 MHz、868 MHz、315 MHz、345 MHz和915 MHz ISM频段上传输数据。
...
默认情况下，rtl_433将接收到的数据打印到终端 - 你需要配置它以发布数据到MQTT，以便Home Assistant可以访问它，这可以在配置文件中的一行代码中完成。

一旦你将rtl_433传感器数据放入MQTT，你将需要帮助Home Assistant发现并理解这些数据。你可以通过以下几种方式做到这一点：

  * 手动在HA中配置`sensors`和`binary_sensors`，并将它们链接到从rtl_433输出的适当的MQTT主题([链接到MQTT主题](https://www.home-assistant.io/integrations/sensor.mqtt/))，
...
要使用这个插件，你需要以下条件：
 ...
 1. [支持rtl_433的SDR转接器](https://triq.org/rtl_433/HARDWARE.html)。
...
## 安装
 ...
 1. 创建一个满足你需求的rtl_433配置文件。如果你在运行Home Assistant OS的计算机之外进行此操作，可能会更好，这样你可以自由地实验并迭代，直到你得到一个配置良好的配置。下面有更多详细信息。

 2. 使用你选择的方法（通过Samba插件、ssh/scp、文件编辑器插件等）将配置文件上传到Home Assistant的"/config"目录。

 3. 安装插件。

 4. 插上你的SDR转接器到运行插件的机器。
...
`retain`选项控制MQTT的`retain`标志是默认启用还是禁用。可以通过在`output`设置中将`retain`设置为`true`或`false`来逐个电台覆盖。

在手动配置时，假设你打算将rtl_433数据放入Home Assistant，配置文件中需要指定的绝对最小信息是[MQTT连接和认证信息](https://triq.org/rtl_433/OPERATION.html#mqtt-output)：
...
假设你只有一个USB转接器连接，并且rtl_433能够自动找到它，我们得到了一个看起来像这样的最小rtl_433配置文件：
