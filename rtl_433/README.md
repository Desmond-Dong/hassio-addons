...
这个插件是一个围绕优秀的 [rtl_433](https://github.com/merbanan/rtl_433) 项目的简单封装，它通过 [支持的 SDR 探针之一](https://triq.org/rtl_433/HARDWARE.html) 接收无线传感器数据，解码并以多种格式（包括 JSON 和 MQTT）输出。无线传感器 rtl_433 主要在 433.92 MHz、868 MHz、315 MHz、345 MHz 和 915 MHz ISM 频段上传输数据。
...
默认情况下，rtl_433 将接收到的数据打印到终端 - 您需要配置它将数据发布到 MQTT，以便 Home Assistant 可以访问它，这可以在配置文件中的一行中完成。

一旦您将 rtl_433 传感器数据获取到 MQTT，您就需要帮助 Home Assistant 发现并理解这些数据。您可以通过以下几种方式完成：
...
要使用这个插件，您需要以下条件：
 ...
 1. [一个受 rtl_433 支持的 SDR 探针](https://triq.org/rtl_433/HARDWARE.html)。

 2. 在插有 SDR 探针的机器上运行的 Home Assistant OS。
...
## 安装
 ...
 1. 创建一个满足您需求的 rtl_433 配置文件。如果您在其他不是运行 Home Assistant OS 的计算机上这样做，可能会更好，这样您就可以自由地实验并迭代，直到您找到一个有效的配置。下面有更多详细信息。

 2. 使用任何您的方法将配置文件上传到 Home Assistant 的 "/config" 目录（通过 Samba 插件、ssh/scp、文件编辑器插件等）。

 3. 安装插件。

 4. 将您的 SDR 探针插入运行插件的机器。
...
`retain` 选项控制 MQTT 的 `retain` 标志是默认启用还是禁用。它可以通过在 `output` 设置中将 `retain` 设置为 `true` 或 `false` 来在每个无线电的基础上覆盖。

在手动配置时，假设您打算将 rtl_433 数据获取到 Home Assistant，配置文件中需要指定的最小信息是 [MQTT 连接和认证信息](https://triq.org/rtl_433/OPERATION.html#mqtt-output)：

```
output      mqtt://HOST:PORT,user=XXXX,pass=YYYYYYY
```

rtl_433 默认监听 433.92MHz，但如果那是您需要的，最好明确指定频率以避免混淆：
...
假设您只连接了一个 USB 探针并且 rtl_433 能够自动找到它，我们得到了一个看起来像这样最小 rtl_433 配置文件：
...
这个插件基于 James Fry 的 [rtl4332mqtt Hass.IO 插件](https://github.com/james-fry/hassio-addons/tree/master/rtl4332mqtt)，它是基于 Chris Kacerguis 的项目：[https://github.com/chriskacerguis/honeywell2mqtt](https://github.com/chriskacerguis/honeywell2mqtt)，而 Chris Kacerguis 的项目又是基于 Marco Verleun 的 rtl2mqtt 镜像：[https://github.com/roflmao/rtl2mqtt](https://github.com/roflmao/rtl2mqtt)。