...
这个插件是一个简单的包装器，围绕着优秀的 [rtl_433](https://github.com/merbanan/rtl_433) 项目，通过 [支持的一种SDR转接器](https://triq.org/rtl_433/HARDWARE.html) 接收无线传感器数据，解码并以多种格式（包括JSON和MQTT）输出。无线传感器rtl_433主要在433.92 MHz、868 MHz、315 MHz、345 MHz和915 MHz ISM频段上传输数据。
...
默认情况下，rtl_433将接收到的数据打印到终端——你需要配置它将数据发布到MQTT，以便Home Assistant可以访问它，这可以在配置文件中的一行代码中完成。

一旦你将rtl_433传感器数据放入MQTT，你还需要帮助Home Assistant发现并理解这些数据。你可以通过以下几种方式完成：

  * 手动配置HA中的`sensors`和`binary_sensors`，并将它们链接到从rtl_433发出的相应MQTT主题（[链接到MQTT主题](https://www.home-assistant.io/integrations/sensor.mqtt/)），
...
要使用这个插件，你需要以下条件：
 ...
 3. 一些由rtl_433支持的无线传感器。支持的全列表协议和设备可以在[rtl_433的README](https://github.com/merbanan/rtl_433/blob/master/README.md)的“支持的设备协议”部分找到。

## 安装

 1. 创建一个rtl_433配置文件，满足你的需求。如果你在运行Home Assistant OS的计算机之外完成此操作，可能会更好，这样你可以自由地实验并迭代，直到你得到一个配置良好的配置。见下文了解更多详情。

 2. 使用任何适合你的方法（通过Samba插件、ssh/scp、文件编辑器插件等）将配置文件上传到Home Assistant的"/config"目录。

 3. 安装插件。

 4. 插入你的SDR转接器到运行插件的机器。
...
rtl_433默认监听433.92MHz，即使你需要这个频率，最好明确指定频率以避免混淆：
...
假设你只有一个USB转接器连接，并且rtl_433能够自动找到它，我们得到了一个看起来像这样最小的rtl_433配置文件：
