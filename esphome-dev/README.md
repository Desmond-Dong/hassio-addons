...
这个插件允许您编写配置，将微控制器直接转变为智能家居设备，**无需编程经验**。
您所需要做的就是编写 YAML 配置文件；其余部分（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 直接生成定制固件。例如，为了包含一个 [DHT22][dht22] 温湿度传感器，您只需在配置文件中包含 8 行 YAML：
