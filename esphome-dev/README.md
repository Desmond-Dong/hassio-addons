...
这个插件允许您编写配置，并通过 Home Assistant 将您的微控制器直接变成智能家居设备，**无需编程经验**。您只需要编写 YAML 配置文件；其余（空中更新、编译）均由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 直接到定制固件。例如，要包含一个 [DHT22][dht22] 温度和湿度传感器，您只需在配置文件中包含 8 行 YAML：
