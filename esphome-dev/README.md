...
这个插件允许您编写配置，并通过 Home Assistant 直接将您的微控制器变成智能家居设备，**无需编程经验**。您需要做的是编写 YAML 配置文件；其余的（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，您可以从几行 YAML 直接到一个定制固件。例如，要包含一个 [DHT22][dht22] 温度和湿度传感器，您只需要在配置文件中包含 8 行 YAML：
