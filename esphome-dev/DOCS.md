...
或者您可以指定 fork 所有者的用户名和分支 `username:branch`，前提是存储库仍然命名为 `esphome`。

如果您需要在镜像更新之前测试 dev 分支上的最新提交，您可以将此处的输入设置为 `dev`。
...
如果设置为 `true`，这将启用 streamer 模式，使 ESPHome 隐藏所有潜在的个人隐私信息。例如 WiFi (B)SSID（可用于找到您的位置）、用户名等。请注意，您需要在您的 YAML 文件中使用 `!secret` 标签来防止这些信息在编辑和验证时显示。