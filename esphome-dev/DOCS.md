...
该插件使用每天凌晨 02:00 UTC 自动构建的 ESPHome 版本，并用于测试开发中的组件。请参考下方的 `esphome_fork` 配置来正确配置插件。一旦您更新了配置，请确保重新构建镜像。
...
如果您需要在镜像更新前测试开发分支上的最新提交，您可以在其中输入 `dev`。
...
请注意，如果您将其设置为 `true`，则必须生成加密的密钥和证书文件。例如，使用 [Let's Encrypt](https://www.home-assistant.io/addons/lets_encrypt/)
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有可能包含私信息的详细信息。例如 WiFi (B)SSID（可用于找到您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来防止在编辑和验证时这些信息显示出来。