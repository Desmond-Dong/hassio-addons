...
请注意，您正在使用的分支或分叉 **必须** 与 ESPHome 开发版保持最新，
否则插件 **将不会启动**。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有潜在的私人信息。例如 WiFi (B)SSID（这些信息可能被用来找到您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来阻止这些信息在编辑和验证时显示。