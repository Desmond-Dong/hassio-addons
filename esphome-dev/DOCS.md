...
请注意，您正在使用的分支或分支 **必须** 与 ESPHome 开发版保持最新，否则添加组件 **将不会启动**。
...
请注意，如果您将此设置为 `true`，您还必须生成加密密钥和证书文件。例如，使用 [Let's Encrypt](https://www.home-assistant.io/addons/lets_encrypt/)
...
在相对 URL 下托管 ESPHome 仪表板，以便它可以在现有的 Web 代理（如 NGINX）下集成。默认值为 `/`。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有潜在的个人敏感信息。例如 WiFi (B)SSID（可用于找到您的位置）、用户名等。请注意，您需要在您的 YAML 文件中使用 `!secret` 标签来阻止这些在编辑和验证时显示出来。