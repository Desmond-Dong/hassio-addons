...
这是 ESPHome add on 的**开发**版本。

要部署生产节点，请使用主流发布的 add on。

该 add on 使用的是每天凌晨02:00 UTC自动构建的 ESPHome 版本，用于测试开发中的组件。请参考下方的 `esphome_fork` 配置以正确配置 add on。一旦您更新了配置，请确保重新构建镜像。
...
请注意，您正在使用的分支或分支**必须**与 ESPHome 开发版本同步，
否则 add-on 将不会启动。
...
如果设置为 `true`，这将启用 streamer 模式，使 ESPHome 隐藏所有潜在的个人隐私信息。例如 WiFi (B)SSID（可能用于找到您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来防止这些信息在编辑和验证时显示。