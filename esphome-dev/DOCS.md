...
从分支或fork中安装 ESPHome。
例如，要测试一个pull请求，使用 `pull/XXXX/head`，其中 `XXXX` 是PR编号，或者你可以指定fork所有者的用户名和分支 `username:branch`，假设仓库名称仍然是 `esphome`。

如果你需要在镜像更新前在开发分支上测试最新提交，你可以在其中输入 `dev`。

请注意，你正在使用的fork或分支 **必须** 与 ESPHome 开发版保持同步，否则add-on **将不会启动**。
...
请注意，如果你将其设置为 `true`，你也必须生成加密的密钥和证书文件。例如，使用 [Let's Encrypt](https://www.home-assistant.io/addons/lets_encrypt/)
...
**注意**: 文件 **必须** 存储在 `/ssl/` 中，这是 Home Assistant 的默认位置。
...
如果设置为 `true`，这将启用 streamer 模式，使 ESPHome 隐藏所有潜在的个人隐私信息。例如 WiFi (B)SSID（可能被用来定位你的位置）、用户名等。请注意，你需要在你 YAML 文件中使用 `!secret` 标签来阻止这些信息在编辑和验证时显示。