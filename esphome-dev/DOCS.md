...
该 add on 使用的是每天凌晨02:00 UTC自动构建的 ESPHome 版本，用于测试开发中的组件。参见下方的 `esphome_fork` 配置以正确配置 add on。一旦你更新了配置，请确保重新构建镜像。
...
如果你需要在镜像更新前测试 dev 分支上的最新提交，你可以在此处输入 `dev`。
...
例如 WiFi (B)SSID（可能被用来找到你的位置）、用户名等。
