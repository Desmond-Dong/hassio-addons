...
该插件使用的是每天凌晨02:00 UTC自动构建的 ESPHome 版本，用于测试开发中的组件。请参考下方的 `esphome_fork` 配置来正确配置插件。一旦更新配置，请确保重新构建镜像。
...
请注意，你正在使用的fork或分支 **必须** 与 ESPHome 开发版同步，
否则插件 **将不会启动**。
...
启用或禁用加密的 SSL/TLS (HTTPS) 连接到此插件的网页服务器。
...
在插件配置中添加此选项，允许你通过将其设置为 `true` 来禁用认证。
...
默认情况下，仪表板使用 mDNS 来检查节点是否在线。这不会跨子网工作，除非你的路由器支持 mDNS 转发或 avahi。

将此设置为 `true` 将使 ESPHome 使用 ICMP ping 请求来获取节点状态。如果所有节点即使连接时也总是显示离线状态，请使用此选项。

### 选项: `streamer_mode`

如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有可能包含私人信息的细节。
例如 WiFi (B)SSID（可能被用来找到你的位置）、用户名等。请注意，你需要在你 YAML 文件中使用 `!secret` 标签来阻止这些信息在编辑和验证时显示。