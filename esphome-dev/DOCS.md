...
该插件使用的是每天凌晨 02:00 UTC 自动构建的 ESPHome 版本，用于测试开发中的组件。请参考下方的 `esphome_fork` 配置以正确配置插件。一旦更新了配置，请确保重新构建镜像。
...
如果您需要在镜像更新前测试开发分支的最新提交，可以在此处输入 `dev`。
...
请注意，如果您将此选项设置为 `true`，您还必须生成加密的密钥和证书文件。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有潜在的个人敏感信息。
例如 WiFi (B)SSID（这些可能被用来找到您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来阻止这些信息在编辑和验证时显示。