...
请注意，您正在使用的分支或分支 **必须** 与 ESPHome 开发同步，
否则添加组件 **将不会启动**。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有可能包含私信息的详细信息。例如 WiFi (B)SSID（可用于找到您的位置）、用户名等。
请注意，您需要在 YAML 文件中使用 `!secret` 标签来阻止这些在编辑和验证时显示。