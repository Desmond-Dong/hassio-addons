...
[引导程序](https://www.zigbee2mqtt.io/guide/getting-started/#onboarding) 允许您在不手动输入附加组件配置页面中的详细信息的情况下设置 Zigbee2MQTT。当使用全新安装（没有配置）启动附加组件时，前端将显示一个快速设置页面，允许您选择各种设置，以便 Zigbee2MQTT 能够启动。

> [!NOTE]
> 成功检测到适配器以从中选择可能因您的设置/网络而异。您可能需要在页面上手动输入这些 [详细信息](https://www.zigbee2mqtt.io/guide/configuration/adapter-settings.html#basic-configuration)。

> [!TIP]
> 您可以使用附加组件配置页面中的切换按钮强制引导程序重新运行（例如，更改适配器）。这将强制引导程序在您成功配置它后运行。完成后请确保禁用它。
...
附加组件将在您的数据路径中创建您的 `configuration.yml` 的备份：`$DATA_PATH/configuration.yaml.bk`。在升级时，您应该使用此备份将相关值填写到新配置中，特别是网络密钥，以避免破坏您的网络并重新配对所有设备。
如果未找到以前的备份，则会在附加组件启动时创建您的配置备份。

# 启用看门狗

要自动在出现软故障（如“适配器断开连接”）的情况下重新启动 Zigbee2MQTT，可以使用看门狗。可以通过在附加组件配置中添加以下内容来启用它：
...
- 根据您的配置，MQTT 服务器配置可能需要包含端口，通常用于 SSL 通信的端口为 `1883` 或 `8883`。例如，对于 Home Assistant 的 Mosquitto 附加组件，可以使用 `mqtt://core-mosquitto:1883`。
- 要查找您暴露的串行端口，请转到 **Supervisor → 系统 → 主机系统 → ⋮ → 硬件**
...
**注意：** 您需要根据您的需求更改 `master` 和 `slave` 选项。默认值将确保 socat 监听端口 `8485` 并将其输出重定向到 `/dev/ttyZ2M`。Zigbee2MQTT 的串行端口设置不会自动设置，需要相应地更改。