...
这个插件的安装过程非常简单，与安装其他 Home Assistant 插件的过程类似。
...
启用或禁用与该插件的网络服务器加密的 SSL/TLS (HTTPS) 连接。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有可能私有的信息。例如 WiFi (B)SSID（可能被用来定位您的位置）、用户名等。请注意，您需要在您的 YAML 文件中使用 `!secret` 标签来防止这些信息在编辑和验证时显示。