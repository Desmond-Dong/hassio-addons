...
将此选项添加到扩展配置中，允许您通过设置为 `true` 来禁用身份验证。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有可能包含隐私信息的内容。例如 WiFi (B)SSID（可用于查找您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来阻止这些内容在编辑和验证时显示。