...
3. 可选：如果您正在使用 SSL/TLS 证书并希望加密与该插件的通信，请将 `ssl` 字段设置为 `true`，并相应地设置 `fullchain` 和 `certfile` 选项。
...
请注意，如果您将此设置为 `true`，则必须生成密钥和证书文件以进行加密。例如使用 [Let's Encrypt](https://www.home-assistant.io/addons/lets_encrypt/)
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有可能包含敏感信息的信息。例如 WiFi (B)SSID（可用于找到您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来阻止这些信息在编辑和验证时显示。