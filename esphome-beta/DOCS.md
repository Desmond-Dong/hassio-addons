...
3. 可选：如果您正在使用 SSL/TLS 证书并希望加密到此插件之间的通信，请在 `ssl` 字段中输入 `true`，并相应地设置 `fullchain` 和 `certfile` 选项。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有可能包含敏感信息的信息。例如 WiFi (B)SSID（可用于找到您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来防止在编辑和验证时这些信息显示出来。