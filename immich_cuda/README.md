...
⚠️ 项目正在非常活跃的开发中。请预期存在错误和变化。不要将其作为存储您照片和视频的唯一方式！（来自开发者）
...
直接从您的手机上以CUDA硬件加速支持的自托管照片和视频备份解决方案。这是带有CUDA支持的Immich变体，它为使用NVIDIA GPU进行机器学习任务提供了硬件加速。

此插件基于imagegenius的[docker镜像](https://github.com/imagegenius/docker-immich)，并启用了CUDA支持以增强性能。

## 硬件要求

- **NVIDIA GPU**：兼容具有CUDA支持的NVIDIA显卡
...
| `DISABLE_MACHINE_LEARNING` | 布尔值 | `false` | 禁用ML功能（不推荐用于CUDA变体） |
...
**数据库设置：**
注意，您需要安装一个单独的postgres插件才能连接数据库。您可以在我仓库中安装postgres插件。
注意在启动之前更改密码；之后将无法更改
