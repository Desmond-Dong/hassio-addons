...
从您的手机直接进行自我托管的照片和视频备份解决方案，并支持 OpenVINO 硬件加速。这是 Immich 的 OpenVINO 启用版本，它为使用 Intel GPU 和 CPU 执行机器学习任务提供硬件加速。
...
| `MACHINE_LEARNING_WORKERS` | 整数 | `1` | ML 工作线程的数量（可以随着 OpenVINO 增加数量） |
