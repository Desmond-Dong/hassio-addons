...
直接从您的手机上托管照片和视频备份解决方案，并支持OpenVINO硬件加速。这是Immich的OpenVINO启用版本，它为使用Intel GPU和CPU执行机器学习任务提供了硬件加速。
...
| `DISABLE_MACHINE_LEARNING` | 布尔值 | `false` | 禁用ML功能（不推荐用于OpenVINO版本） |
| `MACHINE_LEARNING_WORKERS` | 整数 | `1` | ML工作者的数量（可以随着OpenVINO增加） |
