...
从您的手机直接进行自我托管照片和视频备份解决方案，并支持 OpenVINO 硬件加速。这是 Immich 的 OpenVINO 支持版本，它为使用 Intel GPU 和 CPU 执行机器学习任务提供了硬件加速。
...
| `MACHINE_LEARNING_WORKERS` | int | `1` | ML 工作线程的数量（可以随着 OpenVINO 增加数量） |
...
- **环境变量**：请参阅 [向您的添加组件添加环境变量](https://github.com/alexbelgium/hassio-addons/wiki/Add-Environment-variables-to-your-Addon)
...
3. 点击 `保存` 按钮以存储您的配置。
4. 启动添加组件。
5. 检查添加组件的日志，以查看是否一切正常。
6. 仔细配置添加组件以满足您的需求，请参阅官方文档进行配置。

**数据库设置：**
请注意，您需要安装一个单独的 postgres 添加组件才能连接数据库。您可以在我的仓库中安装 postgres 添加组件。
请注意在启动之前更改密码；它之后将不会改变

## 支持

在 github 上创建问题，或在 [home assistant 的线程](https://community.home-assistant.io/t/home-assistant-addon-immich/282108/3) 上提问
