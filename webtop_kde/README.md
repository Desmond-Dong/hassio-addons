...
默认情况下，镜像是基于 abc 用户构建的，我们建议使用这个用户，因为所有的初始化/配置都是基于它的。默认密码也是 abc。如果你想要更改这个密码并在访问界面时需要认证，只需在 webtop 中的图形终端中运行 passwd。然后当访问网页界面时使用路径：

http://localhost:3000/?login=true

应用程序的安装不是持久的，你需要通过插件选项来安装。然而，它们的配置是持久的。
...
DNS_servers: 8.8.8.8,1.1.1.1 # 保持空白以使用路由器的 DNS，或设置自定义 DNS 以避免在本地 DNS 广告过滤器的情况下进行垃圾邮件
localdisks: sda1 # 将你的驱动器的硬件名称挂载到逗号分隔，或其标签。例如。 sda1, sdb1, MYNAS...
...
1. 将我的插件仓库添加到你的 Home Assistant 实例中（在右上角的 Supervisor 插件商店中，或如果你已经配置了我的 HA，点击下面的按钮）
