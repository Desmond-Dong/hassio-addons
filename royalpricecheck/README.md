...
## 描述
当皇家加勒比游轮附加项价格降低时进行通知。可以重新定价游轮，只需饮用套餐、互联网、游览等
...
```
其中 `eyXXX.eyXXX` 字符串是一个Home Assistant长生命令牌。长生命令牌可以在用户Home Assistant个人资料页面的底部“长生命令牌”部分创建。
...
# 添加到侧边栏
由于没有WebUI，它不能显示在侧边栏中。但是你可以将以下代码添加到你的Home Assistant `configuration.yaml`中来通过侧边栏条目显示日志
