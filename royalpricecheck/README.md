...
## 描述
当皇家加勒比游轮附加项价格更便宜时进行通知。可以重新定价游轮，只需喝套餐、互联网、游览等
...
```
其中 `eyXXX.eyXXX` 字符串是一个家助手的长期访问令牌。长期访问令牌可以在用户的家助手个人资料页面的底部“长期访问令牌”部分创建。

更多详情：`https://github.com/caronc/apprise/wiki/Notify_homeassistant`

更多详情：`https://github.com/caronc/apprise` 你可以包含多条URL行来发送电子邮件等
# 添加到侧边栏
由于没有WebUI，因此不能在侧边栏中显示。但是你可以将以下代码添加到你的家助手的`configuration.yaml`中，通过侧边栏条目查看日志
