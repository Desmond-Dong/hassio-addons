...
请注意，每个级别会自动包含更严重级别的日志消息，例如，`debug` 也会显示 `info` 消息。默认情况下，`log_level` 设置为 `info`，这是推荐的设置，除非您正在调试。

## 已知问题和限制

- 这个 add-on 需要核心 MariaDB add-on 版本 2.0 或更高版本。
- 这个 add-on 创建的目的是允许管理官方 Home assistant MariaDB add-on。它无法连接到其他 MySQL 或 MariaDB 服务器。
