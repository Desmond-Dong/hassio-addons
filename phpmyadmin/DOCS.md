...
phpMyAdmin 是一个用于 MySQL 和 MariaDB 的数据库管理工具。通常使用操作（管理数据库、表、列、关系、索引、用户、权限等）可以通过用户界面执行，同时您仍然有直接执行任何 SQL 语句的能力。
...
- 这个 add-on 是创建来允许管理官方 Home assistant MariaDB add-on 的。它无法连接到其他 MySQL 或 MariaDB 服务器。
