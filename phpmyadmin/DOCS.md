...
phpMyAdmin 是一个用于 MySQL & MariaDB 的数据库管理工具。经常使用的操作（管理数据库、表格、列、关系、索引、用户、权限等）可以通过用户界面执行，同时您仍然可以直接执行任何 SQL 语句。

这个插件特别设计用来管理官方 Home Assistant MariaDB 插件。
...
**注意**：_这只是个示例，不要复制粘贴它！创建您自己的配置！_

### 选项：`upload_limit`

默认情况下，上传的大小限制（用于导入等操作）设置为 64MB。可以使用这个选项增加这个限制，例如，`100` 将是 100MB。
