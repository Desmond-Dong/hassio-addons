...
默认情况下，上传的大小限制（用于导入等操作）设置为 64MB。可以使用这个选项来增加这个限制，例如，`100` 将是 100MB。
...
- 这个 add-on 是为了允许管理官方 Home assistant MariaDB add-on 而创建的。它无法连接到其他 MySQL 或 MariaDB 服务器。
