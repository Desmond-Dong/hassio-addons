...
此 add-on 是专门设计用来管理官方 Home Assistant MariaDB add-on 的。
...
默认情况下，上传的大小限制（用于导入等操作）设置为 64MB。可以使用此选项增加该限制，例如，`100` 将是 100MB。
...
- `info`：正常（通常）有趣的事件。
- `warning`：非异常的异常情况。
- `error`：需要立即采取行动的运行时错误。
- `fatal`：出了严重问题。add-on 变得无法使用。

请注意，每个级别会自动包含更严重级别的日志消息，例如，`debug` 也会显示 `info` 消息。默认情况下，`log_level` 设置为 `info`，除非您正在解决问题，否则这是推荐设置。

## 已知问题和限制

- 此 add-on 需要核心 MariaDB add-on 版本 2.0 或更高版本。
- 此 add-on 是创建用来管理官方 Home assistant MariaDB add-on 的。它无法连接到其他 MySQL 或 MariaDB 服务器。
