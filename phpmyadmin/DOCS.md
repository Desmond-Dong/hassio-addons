...
`log_level` 选项控制插件生成的日志输出级别，可以设置为更详细或更简洁，这在处理未知问题时可能很有用。可能的值有：
...
请注意，每个级别都会自动包含更严重级别的日志消息，例如，`debug` 也会显示 `info` 消息。默认情况下，`log_level` 设置为 `info`，除非您正在解决问题，否则这是推荐设置。
...
此仓库使用 GitHub 的发布功能[发布](https://github.com/erik73/hassio-addons/releases)功能维护更改日志。

发布基于[语义版本控制](https://semver.org/spec/v2.0.0.html)，格式为 `MAJOR.MINOR.PATCH`。简而言之，版本将根据以下情况增加：
...
您可以在 GitHub 上[打开问题](https://github.com/erik73/addon-phpmyadmin/issues)。
