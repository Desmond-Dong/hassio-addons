...
这个插件的安装非常简单，与安装任何其他 Home Assistant 插件的方法相同。
...
请注意，每个级别会自动包含更严重级别的日志消息，例如，`debug` 也会显示 `info` 消息。默认情况下，`log_level` 设置为 `info`，除非您正在调试。
...
- [Home Assistant Discord 服务器][discord-ha] 用于 Home Assistant 的一般讨论和问题。
