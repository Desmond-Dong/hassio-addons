...
默认情况下，上传的大小限制（用于导入等操作）设置为 64MB。可以使用此选项增加此限制，例如，`100` 将是 100MB。
...
- 此插件创建用来管理官方 Home assistant MariaDB 插件。它不能连接到其他 MySQL 或 MariaDB 服务器。

## 更改日志与发布

此存储库使用 [GitHub 的发布][发布] 功能维护更改日志。

发布基于 [语义版本控制][语义版本控制]，并使用 `MAJOR.MINOR.PATCH` 格式。简而言之，版本将根据以下情况增加：
