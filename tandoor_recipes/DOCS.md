...
此插件现在使用 Tandoor 的集成 Nginx 服务器，并默认暴露端口 80（映射到 9928）。
...
目录 `/config/addons_config/tandoor_recipes/externalfiles` 可用于将外部文件导入 Tandoor。您可以在 Docker 中将其映射到 `/opt/recipes/externalfiles`。按照这里的指示操作：https://docs.tandoor.dev/features/external_recipes/
