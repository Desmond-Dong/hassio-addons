...
你可以将主题/骨架的用户文件夹放在 /share/codex/www/user 中，
...
| `TZ` | 长格式的时区 | - | `America/Los_Angeles` |
| `CODEX_RESET_ADMIN` | 将管理员用户和密码重置为默认值 | - | `1` |
| `CODEX_SKIP_INTEGRITY_CHECK` | 启动时跳过数据库完整性修复 | - | `1` |
| `csrf_allowed` | 允许访问应用的地址列表，用逗号分隔 | `http://homeassistant.local:8123,https://homeassistant.local:8123` | `http://localhost:8123` |
