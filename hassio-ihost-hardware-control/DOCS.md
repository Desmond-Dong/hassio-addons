...
iHost 硬件控制插件在 iHost 上注册按钮和指示灯作为 Home Assistant 中的实体，允许用户直接在 Home Assistant 中配置按钮和指示灯实体的自动化规则，以更灵活地控制硬件交互。

## 前置条件

使用 **iHost 硬件控制** 插件需要在 iHost 上通过预刷写的 microSD 卡启动和运行 Home Assistant，因为它启用了 iHost 的按钮和指示灯功能。有关更多详细信息，请参阅 [操作指南](https://github.com/iHost-Open-Source-Project/ha-operating-system?tab=readme-ov-file#readme)。
...
由于 iHost 硬件控制插件依赖于 MQTT，当你启动 iHost 硬件控制插件时，Mosquitto 中继插件会自动为你安装和启动，你需要等待几分钟才能在插件列表中看到 Mosquitto 中继插件。
...
请前往设置 -> 设备与服务 -> 点击 MQTT 集成的添加按钮，并等待添加完成。
...
按照上述说明成功安装和启动 **iHost 硬件控制** 插件后，你可以在 **MQTT** 集成中看到 iHost 上的按钮和指示灯。
...
iHost 顶部有 4 个按钮：电源、配对、静音和安全。iHost 侧面的小孔中有一个重置按钮，这 5 个按钮都作为一个名为 **iHost 按钮** 的设备存在，有 5 个 **事件** 实体，如下所示：
...
iHost 顶部的每个按钮都有一个蓝色 LED 指示灯。iHost 侧面有一个 LED 灯带。这 5 个指示灯都作为一个名为 **iHost 指示灯** 的设备存在，有 5 个 **选择** 实体，如下所示：
