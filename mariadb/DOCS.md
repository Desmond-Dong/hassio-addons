...
要授予此用户的权限列表，例如 `SELECT` 和 `CREATE`。[授予权限][grant]。
如果省略，将向用户授予 `ALL PRIVILEGES`。不建议限制 Home Assistant 使用的用户的权限，但如果您想允许其他应用程序查看 recorder 数据，应该创建一个仅限于数据库只读访问权限的用户。

### 选项：`mariadb_server_args`（可选）

一些用户在 Home Assistant 大型数据库的架构更新过程中遇到了 [错误][migration-issues]。
如果有可用的 RAM，定义推荐参数可能会有所帮助。
