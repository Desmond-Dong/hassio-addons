...
本节授予 MariaDB 中用户的权限。[授予权限][grant] 文档。
...
从 [授予权限][grant] 授予此用户的权限列表，例如 `SELECT` 和 `CREATE`。
如果省略，将向用户授予 `ALL PRIVILEGES`。不建议限制 Home Assistant 使用的用户的权限，但如果您希望允许其他应用程序查看 recorder 数据，则应创建一个仅限于数据库只读访问的用户。
