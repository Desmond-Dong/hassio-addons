...
如果省略，则授予用户 `ALL PRIVILEGES`。不建议限制家居助手使用的用户的权限，但如果您希望允许其他应用程序查看记录器数据，则应创建一个仅具有数据库只读访问权限的用户。

### 选项：`mariadb_server_args`（可选）

一些用户在大型数据库上的家居助手模式更新期间遇到了 [错误][migration-issues]。
...
MariaDB 将被家居助手中的 `recorder` 和 `history` 组件使用。有关设置此配置的更多信息，请参阅家居助手的 [recorder 集成][mariadb-ha-recorder] 文档。
