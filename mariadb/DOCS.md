...
MariaDB 服务器插件可以根据您的喜好进行调整。本节描述每个插件的配置选项。
...
授予此用户的权限列表，例如 `SELECT` 和 `CREATE`，从 [授权][grant] 中获取。
如果省略，则向用户授予 `ALL PRIVILEGES`。不建议限制 Home Assistant 使用的用户权限，但如果您希望允许其他应用程序查看 recorder 数据，应创建一个仅具有数据库只读权限的用户。
