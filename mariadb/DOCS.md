...
按照以下步骤将此 add-on 安装到您的系统上：

1. 在 Home Assistant 前端导航到 **设置** -> **add-on** -> **add-on store**。
...
此部分定义 MariaDB 中的创建用户定义。[Create User][createuser] 文档。
...
此部分授予 MariaDB 中的用户权限。[Grant][grant] 文档。
...
授予此用户的权限列表，例如 `SELECT` 和 `CREATE`。[Grant][grant]。
