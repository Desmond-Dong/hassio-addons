...
要授予此用户的权限列表，类似于 `SELECT` 和 `CREATE` 的 [授予权限][grant]。如果省略，将向用户授予 `ALL PRIVILEGES`。限制 Home Assistant 使用的用户的权限是不推荐的，但如果您希望允许其他应用程序查看 recorder 数据，则应创建一个仅限于数据库只读访问的用户。
