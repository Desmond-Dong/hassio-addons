...
MariaDB 服务器添加组件可以根据您的喜好进行调整。本节描述每个添加组件配置选项。
...
要授予此用户的权限列表，从 [授权][grant] 如 `SELECT` 和 `CREATE`。如果省略，将向用户授予 `ALL PRIVILEGES`。不建议限制 Home Assistant 使用的用户的权限，但如果您想允许其他应用程序查看 recorder 数据，应该创建一个仅限于数据库只读访问的用户。

### 选项：`mariadb_server_args`（可选）

一些用户在 Home Assistant 大型数据库架构更新期间遇到 [错误][migration-issues]。
