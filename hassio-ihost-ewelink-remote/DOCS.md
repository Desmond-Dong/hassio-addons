...
如果您已经添加了此仓库的插件（例如 iHost 硬件控制），则可以跳过此步骤，直接进入插件商店安装所需的插件。
1. 通过 URL
...
安装完成后，点击启动以启动插件。等待服务完全启动后再继续操作。
...
- 在点击“配对”以开始添加 eWeLink-Remote 子设备之前，您需要配置并启用蓝牙“**被动扫描**”以检测并连接到 eWeLink-Remote 子设备。如果提前未启用，将出现提示页面。您可以按照提示路径（转到蓝牙集成 > 配置 > 配置蓝牙选项，选中被动扫描，然后点击提交。）来启用它。
...
2. 点击“配对”开始添加设备。
倒计时为 180 秒。在添加过程中，您可以手动点击“退出 xxs”以停止添加设备。在倒计时期间按下子设备的任何按钮以完成设备添加。
**注意*：
- 支持 180 秒内添加多个设备
- 添加智能灯设备的上限为 50 个
...
2. 设备添加成功后，您可以在 Home Assistant 中配置场景，并将场景触发条件设置为场景触发条件。
...
**注意*：
- 在 Home Assistant 自动化中成功配置 eWeLink-Remote 子设备后，如果实体名称被更改，自动化配置信息中将显示为“未知”，但这不会影响自动化场景触发。
