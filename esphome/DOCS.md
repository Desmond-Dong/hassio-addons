...
5. 点击“打开 Web UI”以打开 ESPHome 仪表板。系统将要求您提供 Home Assistant 凭据 - ESPHome 使用 Home Assistant 的认证系统进行登录。
...
**注意**：文件必须存储在 `/ssl/` 中，这是 Home Assistant 的默认值。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有潜在的私人信息。例如 WiFi (B)SSID（可用于找到您的位置）、用户名等。请注意，您需要在您的 YAML 文件中使用 `!secret` 标签来防止在编辑和验证时显示这些信息。