...
**注意**：文件必须在 `/ssl/` 中存储，这是 Home Assistant 的默认设置。
...
将此选项添加到插件配置中，允许您通过设置为 `true` 来禁用认证。
...
如果设置为 `true`，这将启用流模式，使 ESPHome 隐藏所有潜在的私人信息。例如 WiFi (B)SSID（这些SSID可用于找到您的位置）、用户名等。请注意，您需要在 YAML 文件中使用 `!secret` 标签来阻止这些信息在编辑和验证时显示。