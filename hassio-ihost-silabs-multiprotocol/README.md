...
这个插件允许您在单个基于Silicon Labs的无线电设备上同时使用Zigbee和OpenThread协议。该无线电设备需要安装RCP Multi-PAN固件以支持多个IEEE 802.15.4个人区域网络（PAN）。该插件基于Silicon Labs多协议插件进行修改，并在SONOFF ZBDongle-E上成功测试。
...
启用**Silicon Labs Multiprotocol Add-on**后，Zigbee2MQTT在首次启动时可能会报告`ASH_ERROR_TIMEOUTS`错误。通常重新启动Zigbee2MQTT可以解决此问题。此问题已报告给Silicon Labs，目前正在调查中。
