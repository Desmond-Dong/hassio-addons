## 警告：打开问题 : [🐛 [BirdNET-Go] HA Ingress url 不会重定向到带有日期的 url (于 2025-04-02 打开)](https://github.com/alexbelgium/hassio-addons/issues/1830) 由 [@phobiac](https://github.com/phobiac)
...
[![支持](https://img.shields.io/badge/Buy%20me%20a%20coffee%20with%20Paypal%200070BA?logo=paypal&style=flat&logoColor=white)](https://www.paypal.com/donate/?hosted_button_id=DZFULJZTP3UQA)
...
_感谢大家给我的仓库点赞！要点赞请点击下面的图片，然后它将出现在右上角。谢谢！_
...
[BirdNET-Go](https://github.com/tphakala/birdnet-go/tree/main) 是一个由 @tphakala 开发的 AI 解决方案，用于持续鸟类监测和识别。
...
音频片段文件夹可以存储在外部或 SMB 驱动器上，通过从插件选项挂载它，然后指定路径而不是 "clips/"。例如，"/mnt/NAS/Birdnet/"
...
- config.yaml
使用 Filebrowser 插件在 /config/db21ed7f_birdnet-go/config.yaml 中配置 config.yaml 文件，以配置额外的变量
...
1. 将我的插件仓库添加到你的 Home Assistant 实例中（在 Supervisor 插件商店的右上角，或者如果你已经配置了我的 HA，点击下面的按钮）
   ...
   ![打开你的 Home Assistant 实例并显示添加插件仓库对话框，其中指定了特定的仓库 URL](https://my.home-assistant.io/badges/supervisor_add_addon_repository.svg)
   [打开你的 Home Assistant 实例并显示添加插件仓库对话框，其中指定了特定的仓库 URL](https://my.home-assistant.io/redirect/supervisor_add_addon_repository/?repository_url=https%3A%2F%2Fgithub.com%2Falexbelgium%2Fhassio-addons)
1. 安装这个插件。
1. 点击 `保存` 按钮以保存你的配置。
1. 设置插件的选项以符合你的偏好。
1. 启动插件。
...
Systemd 服务文件示例。根据需要调整 user:group。如果你要以 root 用户运行，你可能需要运行 vlc-wrapper 而不是 vlc。
