## 警告：打开问题：[🐛 [Birdnet-go] USB 麦克风在 UI 中可选，但无法工作（于 2025-03-12 打开）](https://github.com/alexbelgium/hassio-addons/issues/1808) 由 [@melor](https://github.com/melor)
...
- config.yaml
可以使用 Filebrowser 插件在 /config/db21ed7f_birdnet-go/config.yaml 中找到的 config.yaml 文件配置附加变量
...
VLC 打开了一个 TCP 端口，但流是 UDP。因此需要配置 Birdnet-Go 使用 UDP。调整 config.yaml 文件为 UDP 或使用 birdnet-go 命令行选项：
