...
Mealie 是一个自托管食谱管理和饮食计划工具，具有 REST API 后端和一个在 Vue 中构建的响应式前端应用程序，为整个家庭提供愉悦的用户体验。
这个 Mealie 1.0 的插件基于 hendrix04 的 [组合 Docker 镜像](https://hub.docker.com/r/hendrix04/mealie-combined)。
这个插件基于 hay-kot 的 [Docker 镜像](https://hub.docker.com/r/hkotel/mealie)。

## 配置

- 启动插件。等待一段时间并检查日志中是否有任何错误。
...
sensor:
  - platform: rest
    ...
    headers:
      Authorization: Bearer <在此处放置 auth>
