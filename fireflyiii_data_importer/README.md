...
["Firefly III"](https://www.firefly-iii.org) 是一个（自托管）的个人财务管理工具。它可以帮助你跟踪支出和收入，以便你花得更少，存得更多。数据导入器旨在帮助你将交易导入 Firefly III。出于安全和维护的原因，它与 Firefly III 分开。

此插件基于 Docker 镜像 https://hub.docker.com/r/fireflyiii/data-importer

## 配置

阅读官方文档以获取有关如何设置变量的信息：https://docs.firefly-iii.org/data-importer。
...
"Updates": 每小时|每天|每周 # 设置 /config/addons_config/fireflyiii_data_importer/import_files 中设置的文件的自动上传
"silent": true # 抑制调试消息
```

- Config.yaml（高级用法）

可以通过在 config.yaml 中添加它们来作为 ENV 变量设置附加变量，根据此指南： https://github.com/alexbelgium/hassio-addons/wiki/Addons-feature:-add-env-variables
...
1. 将我的插件仓库添加到你的 Home Assistant 实例中（在右上角的 Supervisor 插件商店中，或点击下方按钮如果你已经配置了我的 HA）
