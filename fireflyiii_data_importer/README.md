...
["Firefly III"](https://www.firefly-iii.org) 是一个（自托管）的个人财务管理工具。它可以帮助你跟踪支出和收入，这样你就可以少花钱多存钱。数据导入器是为了帮助你将交易导入 Firefly III 而构建的。它出于安全和维护的原因与 Firefly III 分离。

这个插件基于以下 Docker 镜像：https://hub.docker.com/r/fireflyiii/data-importer

## 配置

查阅官方文档以获取有关如何设置变量的信息：https://docs.firefly-iii.org/data-importer。
...
可以通过在 /addon_configs/xxx-fireflyiii_data_importer/import_files 中添加文件来执行自动导入，根据： https://docs.firefly-iii.org/data-importer/usage/command_line/。
...
可以通过在 config.yaml 中添加它们来将附加变量设置为 ENV 变量，位置根据你的插件选项定义，并按照此指南： https://github.com/alexbelgium/hassio-addons/wiki/Add%E2%80%90ons-feature-:-add-env-variables

完整的 ENV 变量列表可以在以下位置查看： https://github.com/firefly-iii/data-importer/blob/main/.env.example
