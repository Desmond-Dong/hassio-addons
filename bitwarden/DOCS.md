...
此插件基于轻量级且开源的 [Vaultwarden][vaultwarden] 实现，允许您自托管这个优秀的密码管理器。
...
1. 点击下方的 Home Assistant 我的按钮，在您的 Home Assistant 实例中打开插件。
...
1. 将 `/admin` 添加到 URL 以访问管理员面板，例如 `http://hassio.local:7277/admin`。使用在第 4 步中获得的管理员令牌登录。
1. 日志中的管理员/令牌仅显示，直到保存或更改。在管理员面板中点击保存，以使用随机生成的密码或将其更改为您选择的密码。
1. 务必将您的管理员令牌保存在安全的地方。**插件将不再显示它！**
...
启用/禁用 SSL（HTTPS）。设置为 `true` 以启用，否则设置为 `false`。
...
默认情况下，API 调用限制为 10MB。这应该对大多数情况足够，但如果您想支持大型导入，这可能限制您。另一方面，您可能希望将请求大小限制为小于该值，以防止 API 滥用和可能的拒绝服务攻击，尤其是在资源有限的情况下运行时。
...
- 一些网页浏览器，如 Chrome，不允许在不安全上下文中使用 Web Crypto API。在这种情况下，您可能会遇到 `Cannot read property 'importKey'` 错误。要解决这个问题，您需要启用 SSL 并使用 HTTPS 访问网页界面。
...
软件按“原样”提供，不提供任何形式的保证，包括但不限于对适销性、特定用途适用性和非侵权性的保证。在任何情况下，作者或版权持有人均不对任何索赔、损害赔偿或其他责任负责，无论是由合同、侵权或其他行为引起的，均与软件的使用或其他交易有关。
