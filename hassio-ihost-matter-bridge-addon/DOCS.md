...
Matter Bridge for iHost 插件将 Home Assistant 设备的实体暴露为 Matter 使能设备，使其能够与 Matter 平台（如 Apple Home、Google Home 和 Amazon Alexa）集成。
...
**Matter Bridge for iHost 插件** 仅设计用于 **HA over iHost** 项目，允许用户将 Home Assistant 设备的实体暴露为 Matter 设备，并将其同步到受支持的 Matter 平台进行控制。

因此，只有当 Home Assistant 在 iHost 上运行时，此插件才可用。学习 [<u>如何在 iHost 上运行 Home Assistant 操作系统？</u>](https://github.com/iHost-Open-Source-Project/ha-operating-system?tab=readme-ov-file#readme)
...
1.  点击 **打开 WEB UI** 以访问 Matter Bridge 配对页面。
   点击 **启动** 查看 Matter Bridge 配对的准备工作。
...
-   点击 **开始配对**。
-   使用任何 Matter 使能平台的 App 扫描 **二维码** 或输入 **数字设置代码** 来配对 Matter Bridge，包括：
    -   Apple Home App
    -   Amazon Alexa App
    -   Google Home App
    -   Samsung SmartThings
...
3. **设备状态未实时更新。**
   在 Alexa 和 Google Home App 中，当从其他平台控制设备时，设备状态不会自动更新。您需要手动刷新设备列表或打开设备详情页面以查看当前状态。

4. **灯光亮度百分比偏差。**
   在 SmartThings App 中，显示的灯光设备亮度百分比始终比实际亮度高约 1%。

5. **窗帘位置百分比跨平台反转。**
   窗帘开启百分比在不同平台上解释不同。例如，在 Alexa 中显示为 30% 开启的窗帘，在 Apple Home、SmartThings 和 Google Home 中会显示为 70% 开启。

6. **在 SmartThings App 中重新同步后设备未显示。**
   如果之前同步的设备被移除并稍后重新同步，它可能不会立即出现在 SmartThings App 中。重新启动 SmartThings Hub 是解决此问题的必要步骤。
...
-   点击右侧的 **全部移除** 可以移除所有已连接的 Matter Fabric。
...
-   选择要同步的实体，或取消选中以从已连接的 Matter Fabric 中移除。
-   点击 **保存**。
