...
- Matter Bridge 插件仅设计用于 HA over iHost 项目，允许用户将 Home Assistant 设备暴露为 Matter 设备，并同步到受支持的 Matter 平台进行控制。
...
由于智能家庭平台在实现 Matter 标准时存在差异，**相同的 Matter 设备在每个生态系统中可能会出现并表现不同**。这包括但不限于：
...
3. **设备状态未实时更新。**
   在 Alexa 和 Google Home 应用中，当设备从其他平台进行控制时，设备状态不会自动更新。您需要手动刷新设备列表或打开设备详情页面以查看当前状态。

4. **灯光亮度百分比偏差。**
   在 SmartThings 应用中，显示的灯光设备亮度百分比始终比实际亮度水平高约 1%。

5. **窗帘位置百分比跨平台反转。**
   窗帘开百分比在不同的平台上被解释不同。例如，在 Alexa 中显示为 30% 开的窗帘，在 Apple Home、SmartThings 和 Google Home 中会显示为 70% 开。

6. **设备在 SmartThings 应用重新同步后未显示。**
   如果之前同步的设备被移除并稍后重新同步，它可能不会立即在 SmartThings 应用中显示。需要重启 SmartThings Hub 来解决此问题。
