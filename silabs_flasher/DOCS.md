...
1. 在 Home Assistant 前端导航到 **设置** -> **插件、备份和主控制器** -> **插件商店**。
2. 找到 "Silicon Labs Flasher" 插件并点击它。
3. 点击 "安装" 按钮。

## 如何使用

此插件需要一个可通过串口访问的 Silicon Labs 基于无线模块（例如 Home Assistant 黄色上的模块、Home Assistant SkyConnect/ZBT-1 或其他基于 USB 的无线适配器）。
...
| flow_control        | 如果硬件流控制应该被启用（取决于固件） |
...
- 加入 Reddit 的 [subreddit][reddit] 在 [/r/homeassistant][reddit]
