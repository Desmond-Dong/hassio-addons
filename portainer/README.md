...
实施更改：更新到最新版本；入口；SSL；通过插件选项设置密码；允许手动覆盖

_感谢所有给我的仓库添加星标的人！点击下面的图片添加星标，它将在右上角显示。谢谢！_
...
Portainer 是一个开源的轻量级管理 UI，允许你轻松管理你的 Docker 主机或 Docker 群集。
...
Portainer 插件非常强大，几乎可以访问你的整个系统。虽然这个插件是经过精心创建和维护的，并且考虑了安全性，但在错误或不熟悉的人手中，它可能会损坏你的系统。
...
1. 将我的插件仓库添加到你的 Home Assistant 实例中（在右上角的监督器插件商店中，或者如果你已经配置了我的 HA，点击下面的按钮）
...
Web UI 可以在 <http://homeassistant:port> 找到，或者在 Ingress 侧边栏中。
默认的用户名/密码：在启动日志中描述。
配置可以通过 Web UI 进行，除了以下选项
