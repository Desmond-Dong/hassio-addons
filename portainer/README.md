...
Portainer 是一个开源的轻量级管理 UI，它允许你轻松管理你的 Docker 主机或 Docker 集群。
...
Portainer 插件非常强大，几乎可以让你访问整个系统。虽然这个插件是精心创建和维护的，并且考虑了安全性，但在错误或不熟练的人手中，它可能会损坏你的系统。
...
1. 将我的插件仓库添加到你的 Home Assistant 实例中（在右上角的 Supervisor 插件商店，或者如果你已经配置了我的 HA，点击下面的按钮）
...
Webui 可以在 <http://homeassistant:port> 找到，或在 Ingress 中使用侧边栏。
默认的用户名/密码：在启动日志中描述。
