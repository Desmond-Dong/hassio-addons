端口ainer 可以用来在 Docker 容器中执行自定义命令。它是一个开源的轻量级管理界面，允许您轻松管理 Docker 主机或 Docker 群集。
...
- 更改用户名，或者更通常的是点击连接
- 输入您的命令，您拥有此特定容器的终端的完全访问权限（这不会影响您 HA 系统的其他部分）

# 对您系统的影响
- 安装或运行 portainer 没有影响
- 手动安装自定义容器将使您的 HA 状态变为不支持/不健康状态。您将无法升级 Home Assistant 和您可能拥有的任何插件。停止此自定义容器将重置正常状态
...
## 60 秒超时
插件包含一个非常长的超时时间。但是，如果您使用另一层代理，例如插件 nginx 代理管理器，它将默认为 60 秒的超时时间。您必须调整代理层以增加超时时间。更多详情请参阅：https://github.com/portainer/portainer/issues/2953#issuecomment-1235795256
