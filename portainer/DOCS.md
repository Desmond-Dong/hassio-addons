Portainer 可以用于在 Docker 容器中执行自定义命令。它是一个开源的轻量级管理界面，允许您轻松管理 Docker 主机或 Docker 群集。
...
- 手动安装自定义容器将使您的 HA 状态变为不受支持/不健康状态。您将无法升级 Home Assistant 和升级您可能有的任何插件。停止此自定义容器将重置正常状态
...
## 60 秒超时
插件包含一个非常长的超时时间。但是，如果您使用另一层代理，例如插件 nginx proxy manager，它将默认设置为 60 秒超时。您需要调整代理层以增加超时时间。更多详情请参阅：https://github.com/portainer/portainer/issues/2953#issuecomment-1235795256

## 进一步参考
- 这里有一个关于如何使用的完整指南：https://codeopolis.com/posts/beginners-guide-to-portainer/
