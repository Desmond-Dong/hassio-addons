...
---
从作者github中提取：
[unpackerr](https://github.com/unpackerr/unpackerr) 在您的下载主机上作为守护进程运行。它会检查完成的下载并将其提取出来，以便Lidarr、Radarr、Readarr、Sonarr可以导入它们。在客户端下载文件后进行提取和删除文件，有许多选项。

此插件基于docker镜像 https://hub.docker.com/r/hotio/unpackerr

## 安装

---

此插件的安装非常简单，与安装任何其他插件的方法没有不同。

1. 将我的插件仓库添加到您的home assistant实例中（在supervisor插件商店的右上角，或点击下面的按钮，如果您已配置我的HA）
...
在 /addon_configs/db21ed7f_unpackerr/unpackerr.conf中，您可以按照此环境变量列表设置所有变量：https://github.com/davidnewhall/unpackerr
