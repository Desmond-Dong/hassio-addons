...
基于 [File Browser](https://filebrowser.org/) 的网页版文件浏览器。
这个插件基于 [docker 镜像](https://hub.docker.com/r/filebrowser/filebrowser) 的 filebrowser。

## 配置

Web 界面位于 <http://homeassistant:port>，在插件的选项中将 8080 映射到此端口。
...
```yaml
ssl: true/false
certfile: fullchain.pem #ssl证书
keyfile: privkey.pem #sslkeyfile
NoAuth: true/false #移除密码。更改时重置数据库。
disable_thumbnails : true/false (设置 disable_thumbnails 为 true 或 false；默认为 true 以提高速度)
localdisks: sda1 #将驱动器的硬件名称挂载到这里，用逗号分隔，或其标签。例如。 sda1, sdb1, MYNAS...
...
1. [将我的 Hass.io 插件仓库][repository] 添加到你的 Hass.io 实例。 [![在我的 Home Assistant 中添加仓库][repository-badge]][repository-url]
1. 安装这个插件。
1. 点击 `保存` 按钮来保存你的配置。
1. 启动插件。
1. 检查插件的日志，看看是否一切正常。
1. 仔细配置插件以符合你的偏好，参考 [官方文档](https://filebrowser.org/configuration) 来进行配置。

## 支持

在 github 上创建问题，或者在 [home assistant 论坛](https://community.home-assistant.io/t/home-assistant-addon-filebrowser/282108/3) 上提问。
