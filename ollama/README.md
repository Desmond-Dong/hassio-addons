...
请注意，此插件使用CPU加速或实验性Nvidia GPU支持运行（如果它对您有效，请报告！）对于ROCm，支持仍然在等待中。

## 模型目录

默认情况下，所有下载的模型都存储在`/share/ollama`。由于历史原因，您也可以将其配置为`/config/ollama`。请确保您有足够的空间。
...
UI链接仅用于检查Ollama的API是否可用。官方Ollama镜像中不包括聊天功能。